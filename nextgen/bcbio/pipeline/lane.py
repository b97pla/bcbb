...
def process_lane(info, fc_name, fc_date, dirs, config):
    ...
    write_multiplex_counts(multiplex,lane_name,dirs,config)
        
    return lane_items


def write_multiplex_counts(multiplex, lane_name, dirs, config):
    if not multiplex:
        return
    
    metrics_file = os.path.join(dirs["work"], "%s_barcode" % lane_name, "%s_bc.metrics" % lane_name)
    dmplx_report_file = os.path.join(dirs["work"], config["local_dmplx_counts"])
    ...
    if os.path.exists(metrics_file):
        ...
        with open(metrics_file,"rb") as mfr:
            ...
            for row in csvr:
                d = {'date': fc_date, 'flowcell_id': fc_name, 'lane': info['lane'], 'description': info.get('description','Lane '+str(info['lane'])), 'barcode_id': row[0], 'barcode_name': barcodes.get(row[0],['.'])[0], 'barcode_sequence': barcodes.get(row[0],['.'])[1], 'read_count': row[1]}
                dmplx.append(d)
